--[[	
	...
	Version - 0.9.7:krul(V4.3.2.0408)
	...
	local VERSION = "0.9.7:krul(V4.3.2.0408)"
	local SelectPlayer = game.Players.LocalPlayer.Name

	-- Physics Tuning Loadup Defaults
	local loadupBreastsStiffness = 8
	local loadupBreastsDamping = 0.207
	local loadupButtsStiffness = 10
	local loadupButtsDamping = 0.1
...
function RoClothes(Player)
	...
	-- Module 3: Loader API Standardization
	local System = {
	...
	-- Module 4: Atomic Execution & State Guard
	function Function.CharacterIsBuilt(Character)
		...
		-- Minimum requirement for Ro-Clothes to begin welding
		return Humanoid ~= nil and Root ~= nil and Character:IsDescendantOf(workspace)
	end

	-- Module 3: Loader API Standardization & Asset Management
	local env = System.Env

	function Function.GetAsset(Path)
		-- Automatically handle both rbxasset and raw local paths
		local RelativePath = Path:gsub("rbxasset://", "")
		-- Use System.isfile/getcustomasset which correctly resolves getcustomasset or getsynasset
	...
	-- variables --
	local VersionInternal = "4.2.4.0406"
	local VersionDate = "2026-04-06"
	local hidden = true
	...
	local CVersion = "0.9.7:krul(V4.3.2.0408)"
	...
	function Function.PlayerDataDefault()
		return {
			Character = nil,
			ExecutionMaid = Maid.new(), -- Module 4: Atomic cleanup for this specific player
			LastRequestID = 0,
...
-- [ SYSTEM / FUNCTIONS ]
	...
	function Function.BunnyBandPin(ObjectInstance, Character, Extra, Data)
		...
		Weld.Part1 = Pin
		Weld.C0 = System.MeshOffsets.BunnyBandPin.C0
	...
	local function setUpTailAccessory(character:Model, accessory:Accessory, Data)
		...
		if not handle then
			...
			until handle or count > 20

			if not handle then
				warn("[RoClothes] setUpTailAccessory: Handle not found after timeout for accessory:", accessory.Name)
				return
			end
		end
		...
		if not weld then
			...
			repeat
				task.wait(0.1)
				count = count + 1
				weld = handle:FindFirstChildOfClass("Weld") or handle:FindFirstChildOfClass("WeldConstraint")
			until weld or count > 10

			if not weld then
				warn("[RoClothes] setUpTailAccessory: Weld not found after timeout for handle:", handle.Name)
				return
			end
		end
	...
	function Function.UIStrokeCreate(Parent)
		...
		UIStroke.Color = Color3.fromRGB(38, 38, 58)
	...
	function Function.CharacterFunction(Character, Data)
		...
		-- V3.2 Fix: Single GetDescendants pass  collect Attachments and Accessories together
		...
		if CParentAttachment then
			...
			detectRemoval = CHandle.ChildRemoved:Connect(function(c)
				if CHandle.Parent ~= nil then
					...
					-- V3.2 Fix: Prune dead entry from AllConnect immediately
	...
	function Function.CharacterExecute(Character, Data, bool, RequestID)
		-- Performance Guard: Atomic thread locking
		local DataDetail = PlayerData[Data]
		if not DataDetail then return end
		
		-- Kill all previous execution threads/meshes for this player immediately
		if DataDetail.ExecutionMaid then
			DataDetail.ExecutionMaid:DoCleaning()
		end

		task.defer(function()
			-- Cancellation Point: If a newer request has started, self-terminate
			...
			if not Character or not Function.CharacterIsBuilt(Character) then
				-- Fallback for extremely laggy loads
				local count = 0
				repeat 
					task.wait(0.2)
					count = count + 1
			...
			-- Cancellation Point: Re-check after wait loop; a newer request may have arrived during the delay
			if RequestID and RequestID ~= DataDetail.LastRequestID then return end
			...
			-- V3.3 Fix: Inline CurrentPartList registration  each organ is tracked immediately after
			-- it is parented so CharacterReset can always find and destroy it. Guards prevent a
			-- stale cancelled thread from overwriting a fresh thread's already-registered value.
	...
	function Function.CharacterConnection(Player)
		local CharacterConnect = Player.CharacterAdded:Connect(function(Character)
			if PlayerData[Player.Name] ~= nil and PlayerData[Player.Name].AutoExecute then
				-- Module 4: Automated IsBuilt check instead of fixed polling
	...
	function Function.GUIUpdate()
		...
		for i, v in pairs(GUIObject.BundlesButtonFrame:GetChildren()) do
			if v:IsA("Frame") then
				...
				elseif v.BackgroundColor3 == Color3.fromRGB(50, 100, 170) then
					v.BackgroundColor3 = Color3.fromRGB(20, 17, 32)
					if v.Name == "nil" then
		...
		if PlayerData[SelectPlayer].AutoExecute then
			...
			GUIObject.AutoExecuteButton.TextStrokeColor3 = Color3.fromRGB(180, 60, 60)
		end
		...
		if PlayerData[SelectPlayer].BundleBodyColor then
			...
			GUIObject.BundleBodyColorButton.TextStrokeColor3 = Color3.fromRGB(180, 60, 60)
		end

		if PlayerData[SelectPlayer].Face then
			GUIObject.FaceButton.TextStrokeColor3 = Color3.fromRGB(70, 190, 100)
		else
			GUIObject.FaceButton.TextStrokeColor3 = Color3.fromRGB(180, 60, 60)
		end

		if PlayerData[SelectPlayer].MeshSizeLock then
			GUIObject.MeshSizeLockButton.TextStrokeColor3 = Color3.fromRGB(70, 190, 100)
		else
			GUIObject.MeshSizeLockButton.TextStrokeColor3 = Color3.fromRGB(180, 60, 60)
		end

		if PlayerData[SelectPlayer].AccessorySizeLock then
			GUIObject.AccessorySizeLockButton.TextStrokeColor3 = Color3.fromRGB(70, 190, 100)
		else
		...
		if PlayerData[SelectPlayer].MeshBasePartInvisible then
			GUIObject.MeshBasePartInvisibleButton.TextStrokeColor3 = Color3.fromRGB(70, 190, 100)
		else
		...
		if PlayerData[SelectPlayer].BodyPartPhysics then
			GUIObject.BodyPartPhysicsButton.TextStrokeColor3 = Color3.fromRGB(70, 190, 100)
		else
		...
		-- Guarded because GUIUpdate can fire before the sliders are built.
	...
	-- V3.2 Fix: FP ignore list cache  consolidated into one table to stay within Lua's 200-register limit
				...
				if lP and lP.Character then
					...
					if char:FindFirstChild("Head") and char:FindFirstChild("HumanoidRootPart") and char:FindFirstChildOfClass("Humanoid") then
						...
						if currentCamera then
							...
							-- V3.2 Fix: Reconnect character signals if character changed; rebuild cache only when dirty
	...
	function Function.GUIFunc()
		...
		local BodyPartPhysicsConnect = GUIObject.BodyPartPhysicsButton.MouseButton1Click:Connect(function()
			...
			Function.GUIUpdate()
			-- Re-execute so SpringCreate runs with the updated flag
			local char = PlayerData[SelectPlayer].Character
			if char then
				PlayerData[SelectPlayer].LastRequestID = PlayerData[SelectPlayer].LastRequestID + 1
				local id = PlayerData[SelectPlayer].LastRequestID
				Function.CharacterExecute(char, SelectPlayer, nil, id)
			end
		end)
			...
			local BBConnect = BButton:FindFirstChildOfClass("TextButton").MouseButton1Click:Connect(function()
				if DetectingBundle == false then
					...
					if (not v.ClothingBundle or v.ClothingBundle == false) and (not v.IsPreset or v.IsPreset == false) then
						...
						-- V3.3 Fix: Full-body bundle switch clears accumulated clothes from prior bundles.
						-- ClothingBundle (add-on type) intentionally skips this so it can layer on top.
						PlayerData[SelectPlayer].CurrentClothes = {}
					end
								...
								local BBConnect = BButton:FindFirstChildOfClass("TextButton").MouseButton1Click:Connect(function()
									if DetectingBundle == false then
										...
										if (not input.ClothingBundle or input.ClothingBundle == false) and (not input.IsPreset or input.IsPreset == false) then
											PlayerData[SelectPlayer].CurrentBundle = BButton.Name
											-- V4.2.9 fix: match normal bundle-click behavior — clear accumulated
											-- clothes from prior bundles on full-body switch. ClothingBundle
											-- (add-on) skips this intentionally so it can layer on top.
											PlayerData[SelectPlayer].CurrentClothes = {}
										end
	...
	GUIObject.UIGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(14, 14, 22)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(14, 14, 22))}
	...
	GUIObject.UIGradient_34.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(22, 22, 28)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(38, 38, 48))}
	...
	GUIObject.UIGradient_3.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(28, 8, 8)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(28, 8, 8))}
	...
	GUIObject.UIGradient_4.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(30, 10, 48)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(30, 10, 48))}
	...
	GUIObject.UIGradient_5.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(15, 28, 45)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(15, 28, 45))}
	...
	GUIObject.UIGradient_A17.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(22, 22, 28)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(38, 38, 48))}
	...
	GUIObject.UIGradient_6.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(22, 8, 42)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(22, 8, 42))}
	...
	GUIObject.UIGradient_7.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(10, 38, 28)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(10, 38, 28))}
	...
	GUIObject.UIGradient_8.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(10, 38, 28)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(10, 38, 28))}
	...
	GUIObject.UIGradient_15.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(15, 28, 45)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(15, 28, 45))}
	...
	GUIObject.UIGradient_9.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(40, 8, 8)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(40, 8, 8))}
	...
	GUIObject.ResetButton.TextStrokeColor3 = Color3.fromRGB(180, 60, 60)
	...
	GUIObject.UIGradient_10.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(40, 8, 8)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(40, 8, 8))}
	...
	GUIObject.ExecuteButton.TextStrokeColor3 = Color3.fromRGB(70, 190, 100)
	...
	GUIObject.UIGradient_11.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(15, 28, 45)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(15, 28, 45))}
	...
	GUIObject.UIGradient_12.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(14, 14, 22)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(14, 14, 22))}
	...
	GUIObject.UIGradient_19.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(14, 14, 22)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(14, 14, 22))}
	...
	GUIObject.M3UIGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(14, 14, 22)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(14, 14, 22))}
	...
	GUIObject.UIGradient_13.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(14, 14, 22)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(14, 14, 22))}
	...
	GUIObject.UIGradient_14.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(14, 14, 22)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(14, 14, 22))}
	...
	GUIObject.UIGradient_16.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(28, 28, 10)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(28, 28, 10))}
	...
	GUIObject.UIGradient_17.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(15, 28, 45)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(15, 28, 45))}
	...
	GUIObject.UIGradient_18.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(8, 30, 30)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(8, 30, 30))}
	...
	GUIObject.UIGradient_20.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(15, 28, 45)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(15, 28, 45))}
	...
	GUIObject.UIGradient_21.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(15, 28, 45)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(15, 28, 45))}
	...
	GUIObject.UIGradient_22.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(15, 28, 45)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(15, 28, 45))}
	...
	GUIObject.UIGradient_23.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(15, 28, 45)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(15, 28, 45))}
	...
	GUIObject.UIGradient_24.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(28, 28, 10)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(28, 28, 10))}
	...
	GUIObject.UIGradient_33.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(14, 14, 22)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(14, 14, 22))}
	...
	GUIObject.UIGradient_32.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(14, 14, 22)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(14, 14, 22))}
	...
	GUIObject.UIGradient_26.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(18, 28, 48)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(18, 28, 48))}
	...
	GUIObject.UIGradient_27.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(18, 28, 48)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(18, 28, 48))}
	...
	GUIObject.UIGradient_28.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(18, 28, 48)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(18, 28, 48))}
	...
	GUIObject.UIGradient_29.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(18, 28, 48)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(18, 28, 48))}
	...
	GUIObject.UIGradient_30.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(18, 28, 48)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(18, 28, 48))}
	...
	GUIObject.UIGradient_31.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(32, 12, 45)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(32, 12, 45))}
	...
	GUIObject.UIGradientCE.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(15, 28, 45)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(15, 28, 45))}
	...
	GUIObject.UIGradientPPM.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(10, 38, 28)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(10, 38, 28))}
	...
	GUIObject.UIGradientRPM.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(10, 38, 28)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(10, 38, 28))}
	...
	GUIObject.STUIGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(10, 38, 28)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(10, 38, 28))}
	...
	GUIObject.NTUIGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(10, 38, 28)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(10, 38, 28))}
	...
	GUIObject.BS1UIGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(10, 38, 28)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(10, 38, 28))}
	...
	GUIObject.BS2UIGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(10, 38, 28)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(10, 38, 28))}
	...
	GUIObject.BS3UIGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(10, 38, 28)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(10, 38, 28))}
	...
	GUIObject.UIGradientUI.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(18, 28, 48)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(18, 28, 48))}
	...
	GUIObject.UIGradient_A1.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(15, 28, 45)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(15, 28, 45))}
	...
	GUIObject.UIGradient_A2.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(15, 28, 45)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(15, 28, 45))}
	...
	GUIObject.UIGradient_A11.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(15, 28, 45)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(15, 28, 45))}
	...
	GUIObject.UIGradient_A16.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(15, 28, 45)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(15, 28, 45))}
	...
	GUIObject.UIGradient_A4.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(8, 30, 30)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(8, 30, 30))}
	...
	GUIObject.UIGradient_A15.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(8, 30, 30)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(8, 30, 30))}
	...
	GUIObject.UIGradient_A3.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(14, 14, 22)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(14, 14, 22))}
	...
	GUIObject.UIGradient_A5.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(10, 38, 28)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(10, 38, 28))}
	...
	GUIObject.UIGradient_A6.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(10, 38, 28)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(10, 38, 28))}
	...
	GUIObject.UIGradient_A7.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(22, 8, 35)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(22, 8, 35))}
	...
	GUIObject.UIGradient_A8.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(22, 8, 35)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(22, 8, 35))}
	...
	GUIObject.UIGradient_A9.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(40, 8, 8)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(40, 8, 8))}
	...
	GUIObject.UIGradient_A10.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(40, 8, 8)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(40, 8, 8))}
	...
	GUIObject.UIGradient_A12.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(35, 25, 8)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(35, 25, 8))}
	...
	GUIObject.UIGradient_A13.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(35, 25, 8)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(35, 25, 8))}
	...
	GUIObject.UIGradient_A14.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(14, 14, 22)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(14, 14, 22))}
	...
	GUIObject.UIGradient_A18.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(15, 28, 45)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(15, 28, 45))}
	...
	GUIObject.UIGradient_A19.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(30, 12, 12)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(30, 12, 12))}
	...
	GUIObject.UIGradient_A20.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(10, 32, 18)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(10, 32, 18))}
	...
	UIGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(12, 18, 28)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(12, 18, 28))}
	...
	UIGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(10, 32, 18)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(10, 32, 18))}
	UIGradient.Rotation = -90
	UICorner.CornerRadius = UDim.new(0.12, 0)
	...
	UIGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(35, 18, 8)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(35, 18, 8))}
	UIGradient.Rotation = -90
	UICorner.CornerRadius = UDim.new(0.12, 0)
	...
	UIGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(10, 20, 42)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(10, 20, 42))}
	UIGradient.Rotation = -90
	...
	UIGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(35, 8, 22)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(35, 8, 22))}
	UIGradient.Rotation = -90
	UICorner.CornerRadius = UDim.new(0.12, 0)
	...
	UIGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0.00, Color3.fromRGB(40, 8, 10)), ColorSequenceKeypoint.new(1.00, Color3.fromRGB(40, 8, 10))}
	UIGradient.Rotation = -90
	UICorner.CornerRadius = UDim.new(0.12, 0)
	...
	function Function.RunInitialization()
		...
		repeat
			...
			debounce_load += 1
		until debounce_load >= 50 or load.IsLoaded == true
		if loadtxt then
			loadtxt:Destroy()
		end
		
		-- Module 3 Fix: Continue execution even if assets aren't fully loaded (prevent self-destruct)
